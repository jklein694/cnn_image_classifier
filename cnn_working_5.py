...
def run(X_, Y_, epochs=10, learning_rate=0.01, image_size=28):
    ...
    h_conv2 = tf.nn.relu(conv2d(h_pool1, w2_conv, padding='SAME') + b2_conv)
    h_pool2 = max_pool_2x2(h_conv2, padding='SAME')

    h_pool2_flat = tf.reshape(h_pool2, [-1, 7 * 7 * 64])
    h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, w3_fc) + b3_fc)
    h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

    dense_layer = tf.add(tf.matmul(h_fc1_drop, out_w), out_b, name='dense_layer')

    predictions = tf.nn.sigmoid(dense_layer, name='predictions')
